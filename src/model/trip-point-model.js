...
export default class TripPointModel extends Observable {
  ...
  async updateTripPoint(updateType, update) {
    ...
    if (index === -1) {
      throw new Error('there is no tripPoint to update');
    }
    ...
    try {
      ...
      throw new Error('error: tripPoint isn\'t updated');
    }
  }
  ...
  async addTripPoint(updateType, update) {
    try {
      ...
      throw new Error('tripPoint is not added');
    }
  }
  ...
  async deleteTripPoint(updateType, update) {
    ...
    if (index === -1) {
      throw new Error('there is no tripPoint to delete');
    }
    ...
    try {
      ...
      throw new Error('error then trying to delete tripPoint');
    }
  }
